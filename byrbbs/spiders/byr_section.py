...
class ByrSectionSpider(scrapy.Spider):
    ...
    start_urls = ['http://bbs.byr.cn/section/ajax_list.json?uid=' + LOGIN_FORMDATA['id'] + '&root=sec-2']
    top_section_name = ['学术科技']
    ...
    def logged_in(self, response):
        item = ByrSectionItem()
        item['top_section_num'] = 3  # 使存储的类别号从1开始
        item['top_section_name'] = self.top_section_name[0]
        yield scrapy.Request(self.start_urls[0], meta={'cookiejar': response.meta['cookiejar'],'item':item}, headers=HEADERS, callback=self.parse)
